...
func (s *sqlcipherFileState) decryptPage(pageNo int, data []byte) ([]byte, error) {
	...
	if s.useHMAC {
		...
		if !s.verifyHMAC(pageNo, ciphertext, mac) {
...
func (s *sqlcipherFileState) encryptPage(pageNo int, plain []byte) ([]byte, error) {
	...
	if s.useHMAC {
		mac := s.computeHMAC(pageNo, encrypted[plainHeader:ivOffset])
		copy(encrypted[ivOffset+aes.BlockSize:], mac)
	}
	return encrypted, nil
}

func (s *sqlcipherFileState) computeHMAC(pageNo int, ciphertext []byte) []byte {
	mac := hmac.New(s.config.hmacHash(), s.hmacKey)
	mac.Write(ciphertext)
	var pageNum [4]byte
	binary.BigEndian.PutUint32(pageNum[:], uint32(pageNo))
	mac.Write(pageNum[:])
	return mac.Sum(nil)
}
